...
import { updateTask } from '../redux/actions/task_actions'
...
function useState() {
    ...
    if (
        ...
        && pendingActions.findIndex(action => (action.column && action.column.id < 0) || (action.task && action.task.id < 0)) == -1
    ) {
    ...
    let reorderTask = (task: Task) => dispatch(updateTask(task))
